...
class rsSubsAttrUI():
    ...
    def __init__(self):
        ...
        self.fromText = cmds.textField(cmds.textField(), edit=True, parent=s_winRowField1)
        cmds.button(label='Pick up', c=self.fromTextLoad, parent=s_winRowField1)
        ...
        self.toText = cmds.textField(cmds.textField(), edit=True, parent=s_winRowField2)
        cmds.button(label='Pick up', c=self.toTextLoad, parent=s_winRowField2)
        ...
        cmds.separator(height=5, style="none", hr=True, parent=s_winColPro2)
        self.rename = cmds.button(label='Substitute or Clone', w=100, c=self.rename, parent=s_winColPro2)
        cmds.window(self.window, e=1, w=430, h=103)
        cmds.showWindow(self.window)
        cmds.window(self.window, edit=True, widthHeight=(i_windowSize))

    ##
    # FromTextLoad function.
    # @param self: Object pointer.
    # @param argList: Command arguments.
    # @return none
    def fromTextLoad(self, *args):
        ...
        if l_channelSelect != None:
            cmds.textField(self.fromText, e=True, text="%s.%s" % (sel[0], l_channelSelect[0]))
        else:
            if len(sel) > 0:
                cmds.textField(self.fromText, e=True, text=sel[0])
    ...
    def toTextLoad(self, *args):
        ...
        l_channelSelect = cmds.channelBox("mainChannelBox", q=True, sma=True)
        if l_channelSelect != None:
            cmds.textField(self.toText, e=True, text="%s.%s" % (sel[0], l_channelSelect[0]))
        else:
            if len(sel) > 0:
                cmds.textField(self.toText, e=True, text=sel[0])
    ...
    def rename(self, *args):
        fromText = cmds.textField(self.fromText, q=True, tx=True)
        toText = cmds.textField(self.toText, q=True, tx=True)
        b_delSrcAttr = cmds.checkBox(self.delAttr, q=True, v=True)
        rsSubsParam(fromText, toText, b_delSrcAttr)
